...
const ProjectCard: React.FC<ProjectCardProps> = ({ project }) => {
  ...
  return (
    ...
    >
      ...
      <div className="w-full lg:w-[40%] h-64 lg:h-[24rem] relative overflow-hidden bg-pri border-r border-border">
        {/* Adjusted overlay for light mode compatibility */}
        <div className="absolute inset-0 bg-pri/5 z-10 group-hover:bg-transparent transition-all duration-500" />
        <img 
          src={project.banner} 
          alt={project.title} 
        ...
        <div className="absolute top-0 left-0 z-20 bg-card/90 backdrop-blur-md border-r border-b border-border px-4 py-2 flex items-center gap-2">
            {isMobile ? (
                <Mobile size={14} color='currentColor' className="text-sec" />
            ) : (
                <Monitor size={14} color='currentColor' className="text-sec" />
            )}
      ...
      <div className="flex-1 flex flex-col bg-card backdrop-blur-sm">
        ...
        {/* Header Strip - Updated: border-zinc-800 -> border-border, bg-zinc-950/50 -> bg-card/50 */}
        <div className="flex items-center justify-between border-b border-border px-6 py-3 bg-card/50">
           ...
           <div className="text-sec/60 font-mono text-[10px] uppercase">{project.timeline}</div>
        </div>
        ...
        <div className="p-6 md:p-8 flex flex-col h-full relative">
            ...
            {/* Updated: text-zinc-400 -> text-sec, border-zinc-800 -> border-border */}
            ...
            <div className="mt-auto flex flex-col gap-6 pt-6">
                {/* Tech Stack - Updated: text-zinc-600 -> text-sec/70 */}
                <div className="flex items-center gap-3 text-sec/70 flex-wrap">
                    {project.stack && project.stack.map((tech, idx) => (
                        <div key={idx} title={tech.name} className="hover:text-purple-500 transition-colors cursor-help">
                           <tech.icon size={18} />
                        </div>
                    ))}
                </div>
                ...
                {/* Actions */}
                {/* Updated: border-zinc-800/50 -> border-border */}
                <div className="flex items-center gap-4 pt-4 border-t border-border">
                    {/* View Details Button - Updated: bg-white text-black -> bg-text text-pri */}
                    <button 
                      onClick={(e) => {
                        e.stopPropagation(); 
                    ...
                    {/* Live Link - Updated: border-zinc-700 -> border-border, bg-zinc-900 -> bg-card */}
                    {project.liveLink && (
                      <a 
                        ...
                        className="flex items-center justify-center w-12 h-10 border border-border hover:border-purple-500 text-sec hover:text-purple-500 transition-all rounded-sm bg-card shadow-sm"
                      >
